  ...
  // to sequential reference solution for the exact clamping semantics you should follow.
    ...
    for (int r = 0; r < filterWidth; r++) {
      for (int c = 0; c < filterWidth; c++) {
        posx = thread_2D_pos.x + c - filterWidth / 2;
        posy = thread_2D_pos.y + r - filterWidth / 2;
        if (posx < 0) posx = 0;
        if (posx >= numCols) posx = numCols - 1;
        if (posy < 0) posy = 0;
        if (posy >= numRows) posy = numRows - 1;
        sum += static_cast<float>inputChannel[posy * numCols + posx] * filter[r * filterWidth + c];
      }
    }
                        ...
                        const int filterWidth)
{
  ...
  // shareSize = sizeof(float) * filterWidth * filterWidth;
